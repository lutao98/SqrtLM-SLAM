...
KeyFrame::KeyFrame(Frame &F, Map *pMap, KeyFrameDatabase *pKFDB):
    ...
    // add for lidar
    corner_points_sharp_(F.corner_points_sharp_),
    corner_points_less_sharp_(F.corner_points_less_sharp_),
    surface_points_flat_(F.surface_points_flat_),
    surface_points_less_flat_(F.surface_points_less_flat_),
    surface_points_flat_normal_(F.surface_points_flat_normal_),
    surface_points_less_flat_normal_(F.surface_points_less_flat_normal_)
{
    // 获取id
    mnId=nNextId++;
...
void KeyFrame::UpdateConnections()
{
    ...
    stable_sort(vPairs.begin(),vPairs.end(),[](const pair<int,KeyFrame*> a, const pair<int,KeyFrame*> b)
                                            {   if(a.first!=b.first)  return a.first<b.first;
                                                else return a.second->mnId<b.second->mnId;   });         // sort函数默认升序排列
    // 选择ID升序是因为靠的近关键帧的在后面，在下面倒一下后就到了前面
...
void KeyFrame::SetBadFlag()
{
...

    {
        ...
        // Step 4.5 如果还有子节点没有找到新的父节点，感觉有问题，这个if应该不会进去了，上面已经有一个if了，mpparant是第一个加进去的
        ...
        // 如果当前的关键帧要被删除的话就要计算这个,表示原父关键帧到当前关键帧的位姿变换（P->C）
