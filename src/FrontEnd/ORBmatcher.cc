...
int ORBmatcher::SearchByProjection(Frame &F, const vector<MapPoint*> &vpMapPoints, const float th)
{
    ...
    for(size_t iMP=0; iMP<vpMapPoints.size(); iMP++)
    {
        ...
        if(bestDist<=TH_HIGH)
        {
            ...
            nmatches++;
//            std::cout << pMP->mnId << std::endl;  // 优化结果不一样的原因是因为点不一样
        }
    }
                                       ...
                                       vector<pair<size_t, size_t> > &vMatchedPairs, const bool bOnlyStereo)
{
    ...
    // We perform具体来说，pKF1图像的每个特征点与pKF2图像同一node节点的所有特征点依次匹配，判断是否满足对极几何约束，满足约束就是匹配的特征点 the matching over ORB that belong to the same vocabulary node (at a certain level)
...
//    std::cout << nmatches << std::endl;
    ...
    if(mbCheckOrientation)
    {
        ...
        ComputeThreeMaxima(rotHist,HISTO_LENGTH,ind1,ind2,ind3);
//        std::cout << ind1 << " " << ind2 << " " << ind3 << std::endl;
