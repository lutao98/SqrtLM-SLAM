                       ...
                       mnIterations(0), mnBestInliers(0), mbFixScale(bFixScale)
{
    ...
    // 我觉得进行这一步的目的,应该是在局部优化或者全局优化后,地图点的位置也好,
    // 关键帧的位姿也好都会发生变化,为了能够利用上之前优化之后的关键帧位姿和地图点位置,这里就
    // 进行了这样的一个操作
...
void Sim3Solver::SetRansacParameters(double probability, int minInliers, int maxIterations)
{
    ...
    mRansacMinInliers = minInliers;         // 退出RANSAC所需要的最少内点个数;为了方便下面程序中的理解,这里也可以看做是,
                                            // 我们认为的,这堆匹配关系中包含的真正的内点个数
     ...
     * \n 为了计算Sim3,我们需要从这N对匹配点中取三对点;那么如果我从这些点中抽取三对点,
     *    不严谨地我们认为三对点是可以重复的,那么取这三对点均为内点的概率是 p0=epsilon^3
     * \n (上面说不严谨是因为实际上我们是要取若干对不同的点来计算sim3,这几对匹配点肯定不能够是相同的;
     *    但是毕竟我们用于计算sim3的点的对数不是非常多,相比N来说特别小.所以可以
     ...
     * \n 但是这里程序中有个问题:p我们是不知道的! 然后程序中选择直接从外部给定,就是mRansacProb ...
     *    所以我觉得虽然有理论支持,但是这里的理论值还是不准确的
     */
     ...
     *    这种情况下是考虑到可能我们的上层应用不需要一个特别优秀的结果,差不多就可以了,但是
