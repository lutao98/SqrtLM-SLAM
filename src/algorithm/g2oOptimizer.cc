...
#include "algorithm/g2oOptimizer.h"
...
PointIRT PointI2PointIRT(const PointI &Pi){
  PointIRT Pirt;
  Pirt.x = Pi._PointXYZI::x;
  Pirt.y = Pi._PointXYZI::y;
  Pirt.z = Pi._PointXYZI::z;
  Pirt.intensity = Pi._PointXYZI::intensity;
  return Pirt;
}

PointI PointIRT2PointI(const PointIRT &Pirt){
  PointI Pi;
  Pi._PointXYZI::x=Pirt.x;
  Pi._PointXYZI::y=Pirt.y;
  Pi._PointXYZI::z=Pirt.z;
  Pi._PointXYZI::intensity=Pirt.intensity;
  return Pi;
}
 ...
 */
void g2oOptimizer::GlobalBundleAdjustemnt(Map* pMap, int nIterations, bool* pbStopFlag,
 ...
 */
void g2oOptimizer::BundleAdjustment(const vector<KeyFrame *> &vpKFs, const vector<MapPoint *> &vpMP,
 ...
 * @return  inliers数量
 */
int g2oOptimizer::PoseOptimization(Frame *pFrame, PointICloudPtr local_lidarmap_cloud_ptr,
 ...
 */
void g2oOptimizer::LocalBundleAdjustment(KeyFrame *pKF, bool* pbStopFlag, Map* pMap, const lidarConfig* lidarconfig)
{
    ...
    PointIRTCloudPtr local_less_flat_map=PointIRTCloud().makeShared();
    // Keyframes
    for(list<KeyFrame*>::iterator lit=lLocalKeyFrames.begin(), lend=lLocalKeyFrames.end(); lit!=lend; lit++)
    {
        // 当前关键帧不能加进去
        if(pKF->mnId==(*lit)->mnId )
            continue;
        ...
        cv::Mat KFpose=Converter::toCvMat(SE3quat).inv();
        PointIRTCloud corner_points_less_sharp = pKFi->corner_points_less_sharp_;
        PointIRTCloud surface_points_less_flat = pKFi->surface_points_less_flat_;

        Eigen::Matrix<double,4,4> eigenKFipose;
        eigenKFipose << KFpose.at<float>(0,0), KFpose.at<float>(0,1), KFpose.at<float>(0,2),KFpose.at<float>(0,3),
        ...
        Eigen::Affine3d KFSE3(eigenKFipose);
    ...
    std::cout << "                 ::局部角点地图:" << local_less_corner_map->size()
              << "         局部平面点地图:" <<  local_less_flat_map->size() << std::endl;

    // add lidar edge in current keyframe

    int corner_correspondence = 0;
    int plane_correspondence = 0;

    g2o::VertexSE3Expmap* vSE3_curKF = static_cast<g2o::VertexSE3Expmap*>(optimizer.vertex(pKF->mnId));
    g2o::SE3Quat SE3quat_curKF = vSE3_curKF->estimate();
    cv::Mat pose_temp = Converter::toCvMat(SE3quat_curKF).inv();
    Eigen::Matrix<double,4,4> eigenKFpose;
    eigenKFpose << pose_temp.at<float>(0,0), pose_temp.at<float>(0,1), pose_temp.at<float>(0,2),pose_temp.at<float>(0,3),
        pose_temp.at<float>(1,0), pose_temp.at<float>(1,1), pose_temp.at<float>(1,2),pose_temp.at<float>(1,3),
        pose_temp.at<float>(2,0), pose_temp.at<float>(2,1), pose_temp.at<float>(2,2), pose_temp.at<float>(2,3),
 ...
 */
void g2oOptimizer::OptimizeEssentialGraph(Map* pMap, KeyFrame* pLoopKF, KeyFrame* pCurKF,
                                       ...
                                       const bool &bFixScale)
{
    ...
    for(size_t i=0, iend=vpKFs.size(); i<iend;i++)
    {
        ...
        {
            // 如果该关键帧在闭环时没有通过Sim3传播调整过，用自身的位姿
    ...
    // Set Loop edges
    // Step 3：添加边：LoopConnections是闭环时因为地图点调整而出现的新关键帧连接关系（不包括当前帧与闭环匹配帧之间的连接关系）
    for(map<KeyFrame *, set<KeyFrame *> >::const_iterator mit = LoopConnections.begin(), mend=LoopConnections.end(); mit!=mend; mit++)
    {
        ...
        for(set<KeyFrame*>::const_iterator sit=spConnections.begin(), send=spConnections.end(); sit!=send; sit++)
        {
            ...
            if((nIDi!=pCurKF->mnId || nIDj!=pLoopKF->mnId)   // 不包括“当前关键帧”和“闭环关键帧”，在他们之外新增加的连接关系
            ...
            // ?根据两个Pose顶点的位姿算出相对位姿作为边，那还存在误差？优化有用？因为闭环MapPoints调整新形成的边不优化？
    ...
    for(size_t i=0, iend=vpKFs.size(); i<iend; i++)
    {
        ...
        // Loop edges
        // Step 4.2：添加在CorrectLoop函数中AddLoopEdge函数添加的闭环连接边（当前帧与闭环匹配帧之间的连接关系）(这里面也包括了当前遍历到的这个关键帧之前曾经存在过的回环边)
 ...
 */
int g2oOptimizer::OptimizeSim3(KeyFrame *pKF1, KeyFrame *pKF2, vector<MapPoint *> &vpMatches1,
