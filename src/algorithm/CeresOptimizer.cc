...
#include "algorithm/CeresOptimizer.h"
...
void CeresOptimizer::GlobalBundleAdjustemnt(Map *pMap, int nIterations, bool *pbStopFlag,
    ...
    for (int i = 0; i < pFrame->N; i++)
    {
      ...
      if (!pFrame->mvbOutlier[i] && pMP)
      {
        ...
        ceres::CostFunction *cost_function =
            PoseLieCostFunction::Create(p3d, p2d, sqrt( pFrame->mvInvLevelSigma2[kpUn.octave] ));
...
//    std::cout << summary.FullReport() << '\n';
    ...
    for (int i = 0; i < pFrame->N; i++)
    {
      ...
      PoseLieCostFunction err(p2d, p3d,pFrame->mvInvLevelSigma2[kp.octave]);
      double residual[2];
...
void addPosegraphResidual(VecSim3& vScw, const VecSim3& vNCScw,
                          ceres::Problem& problem, int i, int j)
{
  const Sim3d Sji = vNCScw[j] * vNCScw[i].inverse();
  problem.AddResidualBlock(Sim3CostFunction::createCost(Sji), nullptr,
                           vScw[i].data(), vScw[j].data());
}
...
int CeresOptimizer::OptimizeSim3(KeyFrame *pKF1, KeyFrame *pKF2, vector<MapPoint *> &vpMatches1,
