...
#include "algorithm/Initializer.h"
    ...
    mnLastRelocFrameId(0)                               //恢复为0,没有进行这个过程的时候的默认值
{
    ...
    if(sensor==System::STEREO || sensor==System::RGBD || sensor==System::FUSION)
    {
    ...
    local_lidarmap_cloud_ptr_ = PointICloud().makeShared();
    kdtree_local_map_ = pcl::KdTreeFLANN<PointI>().makeShared();

}


cv::Mat Tracking::getDepthimg(){ return Depthimg_.clone();}
cv::Mat Tracking::getRangeImage(){ return range_img_visual_.clone(); }
PointIRTCloud Tracking::getCornerPoints(){ return corner_points_sharp_; }
PointIRTCloud Tracking::getFlatPoints(){ return surface_points_flat_; }
PointIRTCloud Tracking::getLessCornerPoints(){ return corner_points_less_sharp_; }
PointIRTCloud Tracking::getLessFlatPoints(){ return surface_points_less_flat_; }
PointICloud Tracking::getLidarLocalMap() {
  PointICloud local_lidarmap_cloud=*local_lidarmap_cloud_ptr_;
  return local_lidarmap_cloud;
}
const lidarConfig* Tracking::getLidarConfig(){ return  mpSystem->getLidarConfig(); };


std::set<int> Tracking::getTrackingLocalKFid(){
  std::set<int> localKFid;
  for(KeyFrame* KF:mvpLocalKeyFrames){
    localKFid.insert(KF->mnId);
  }
  return localKFid;
}
...
cv::Mat Tracking::GrabImageFusion(const cv::Mat &im,
    ...
    // 构造Frame
    // 对于图像 提取特帧点+利用lidar估计特帧点深度
    // 对于点云 线束化+深度图转化+特征点提取
    mCurrentFrame = Frame(mImGray,                //灰度图像
                          lidar_inputPtr,         //lidar
                          mpSystem->getLidarConfig(),
                          timestamp,              //时间戳
    ...
    surface_points_flat_=mCurrentFrame.getFlatPoints();
    corner_points_less_sharp_=mCurrentFrame.getLessCornerPoints();
    surface_points_less_flat_=mCurrentFrame.getLessFlatPoints();
...
void Tracking::TrackFusion(){
    ...
    if(mState==NOT_INITIALIZED){
        /*if(mSensor==System::FUSION)*/{
            FusionInitialization();
        }
        //这个状态量在上面的初始化函数中被更新
        if(mState!=OK)
            return;
    }else{
        std::cout << "[Tracking](1)::初始化成功!" << std::endl;
...
//        mbOnlyTracking = true;
        if(!mbOnlyTracking){
            ...
            if(mState==OK){
                ...
                local_lidarmap_cloud_ptr_->clear();
                ...
                {
                    std::cout << "[Tracking](2)::运动模型是空（刚初始化），用最近的关键帧来跟踪当前的普通帧!" << std::endl;
                    ...
                    std::cout << "[Tracking](2)::恒速模型跟踪，用最近的普通帧来跟踪当前的普通帧!" << std::endl;
                    // 用最近的普通帧来跟踪当前的普通帧
                    // 根据恒速模型设定当前帧的初始位姿
                    // 通过投影的方式在参考帧中找当前帧特征点的匹配点
                    // 优化每个特征点所对应3D点的投影误差即可得到位姿
                    bOK = TrackWithMotionModel();          // 大部分是这种情况
                    if(!bOK)
                        // 根据恒速模型失败了，只能根据参考关键帧来跟踪
                        bOK = TrackReferenceKeyFrame();
                }
            }else{
                std::cout <<"[Tracking]::LOST!! , Relocalization()" << std::endl;
            }
        }else{
            std::cout <<"[Tracking]:: Localization Mode: Local Mapping is deactivated." << std::endl;
        }
        ...
        if(!mbOnlyTracking){
            if(bOK){
                std::cout << "[Tracking](3)::现在对local map进行跟踪得到更多的匹配，并优化当前位姿!" << std::endl;
                bOK = TrackLocalMap();
            }
        }
        else{
        ...
        if(bOK){
            ...
            {
                std::cout << "[Tracking](4)::跟踪成功，更新恒速运动模型!" << std::endl;
                ...
                // 否则速度为空
                mVelocity = cv::Mat();
                std::cout << "[Tracking](4)::跟踪成功，更新恒速运动模型!速度为空" << std::endl;
            }
            ...
            std::cout << "[Tracking](5)::判断是否插入关键帧！" << std::endl;
            if(NeedNewKeyFrame()){
                CreateNewKeyFrame();
                std::cout << "             ::插入关键帧!   关键帧数量:" << mpMap->KeyFramesInMap() << std::endl;
            }else{
                std::cout << "             ::CreateNewKeyFrame()  不执行操作" << std::endl;
                std::cout << "             ::不插入关键帧!   关键帧数量:" << mpMap->KeyFramesInMap() << std::endl;
            }
...
/// 激光视觉融合地图初始化
void Tracking::FusionInitialization(){
    ...
    if(mCurrentFrame.N>500){
        ...
        std::cout << "[Tracking](0)::新地图创建了 " << mpMap->MapPointsInMap() << " 个地图点!" << std::endl;
...
void Tracking::MonocularInitialization()
{
    ...
    else    // 如果单目初始化器已经被创建
    {
        ...
        // NOTICE 只有 连续 两帧的特征点个数都大于100时，才能继续进行初始化过程
...
bool Tracking::TrackReferenceKeyFrame()
{
    ...
    std::cout << "             ::TrackReferenceKeyFrame() 特帧点匹配数量为:" << nmatches << std::endl;

    // 这里的匹配数超过15才继续
    if(nmatches<15){
        std::cout << "             ::TrackReferenceKeyFrame() 特帧点匹配数量太少!" << std::endl;
        return false;
    }
    ...
    // Step 4:通过优化3D-2D的重投影误差来获得位姿
    Optimizer::PoseOptimization(&mCurrentFrame, local_lidarmap_cloud_ptr_, kdtree_local_map_, mpSystem->getLidarConfig());
    ...
    std::cout << "             ::TrackReferenceKeyFrame() 内点数量为:" << nmatchesMap << std::endl;
...
//    std::cout << pRef->mnId << std::endl;
//    std::cout << pRef->GetPose() << std::endl;
//    /// pRef->GetPose() 会存在每次运行不一样的情况！！！

    // 如果上一帧为关键帧，或者单目的情况，则退出
    if(mnLastKeyFrameId==mLastFrame.mnId || mSensor==System::MONOCULAR){
        std::cout << "             ::TrackWithMotionModel()->UpdateLastFrame() 上一帧为关键帧，不生成临时地图点 " << std::endl;
        return;
    }
    ...
    std::cout << "             ::TrackWithMotionModel()->UpdateLastFrame() 生成临时地图点数量为:"
              << mlpTemporalPoints.size() << std::endl;
...
bool Tracking::TrackWithMotionModel()
{
    ...
    // 根据恒速模型估计当前帧初始位姿  mVelocity在这里是上上帧到上一帧的变换  就是速度的意思  再乘以上一帧的位姿就是预测这一帧的位姿
    ...
    std::cout << "             ::TrackWithMotionModel() 地图点匹配数量为： " << nmatches << std::endl;
    ...
    if(nmatches<20)
    {
        ...
        std::cout << "             ::TrackWithMotionModel() 地图点匹配数量（扩大半径后）为： " << nmatches << std::endl;
    }
    ...
    // Step 3：优化当前帧位姿
    int inliers_num = Optimizer::PoseOptimization(&mCurrentFrame, local_lidarmap_cloud_ptr_, kdtree_local_map_, mpSystem->getLidarConfig());
    std::cout << "             ::TrackWithMotionModel()->PoseOptimization() 内点数量: "<< inliers_num << std::endl;
    ...
    std::cout << "             ::TrackWithMotionModel() 内点数量为： " << nmatchesMap << std::endl;
...
bool Tracking::TrackLocalMap()
{
    ...
    // Step 3：更新局部所有MapPoints后对位姿再次优化
    int inliers_num = Optimizer::PoseOptimization(&mCurrentFrame, local_lidarmap_cloud_ptr_,
                                                  kdtree_local_map_, mpSystem->getLidarConfig());
    std::cout << "             ::TrackLocalMap()->PoseOptimization() 内点数量: "<< inliers_num << std::endl;
    mnMatchesInliers = 0;
    ...
    std::cout << "             ::TrackLocalMap() 跟踪的地图点数量: " << mnMatchesInliers << std::endl;
...
bool Tracking::NeedNewKeyFrame()
{
    ...
    if(mpLocalMapper->isStopped() || mpLocalMapper->stopRequested()){
        std::cout << "             ::NeedNewKeyFrame() 如果局部地图线程被闭环检测使用，不插入关键帧！" << std::endl;
        return false;
    }
    ...
    if( mCurrentFrame.mnId < mnLastRelocFrameId + mMaxFrames && nKFs>mMaxFrames){
        std::cout << "             ::NeedNewKeyFrame() 距离上一次重定位比较近，并且关键帧数目超出最大限制，不插入关键帧！" << std::endl;
        return false;
    }
    ...
    // 单目时，为false
    bool bNeedToInsertClose = (nTrackedClose<125) && (nNonTrackedClose>70);
//    bool bNeedToInsertClose = (nTrackedClose<100) && (nNonTrackedClose>70);
    ...
    std::cout << "             ::NeedNewKeyFrame()  A => " << bNeedToInsertClose << "  深度(近处的点)阈值：" << mThDepth
              << "  跟踪到比较近的三维点数量:" << nTrackedClose
              << " (<100)  且没有跟踪到比较近的三维点数量:" << nNonTrackedClose << " (>70)" << std::endl;
    std::cout << "             ::NeedNewKeyFrame()  B => " << c1a << "  很长时间没有插入关键帧" << std::endl;
    std::cout << "             ::NeedNewKeyFrame()  C => " << c1b << "  满足插入关键帧的最小间隔并且局部地图处于空闲状态"
              << std::endl;
    std::cout << "             ::NeedNewKeyFrame()  D => " << c1c << "  当前帧跟踪到的点比参考关键帧的0.25倍还少:"
              << (mnMatchesInliers<nRefMatches*0.25) << " (" << mnMatchesInliers << "<" << nRefMatches << "*0.25)"
              << "   或满足条件A:" << bNeedToInsertClose << std::endl;
    std::cout << "             ::NeedNewKeyFrame()  E => " << c2 << "  参考帧相比当前跟踪到的点太少:"
              << (mnMatchesInliers<nRefMatches*thRefRatio)
              << " (" << mnMatchesInliers << "<" << nRefMatches << "*"<< thRefRatio <<")"
              << "   或满足条件A:" << bNeedToInsertClose
              << "   同时跟踪到的内点还不能太少:" << mnMatchesInliers << " (>15)" << std::endl;
    std::cout << "             ::NeedNewKeyFrame()  (B||C||D)&&E => " << ((c1a||c1b||c1c)&&c2) << std::endl;
    ...
    if(mSensor!=System::MONOCULAR)
    {
        ...
        if(!vDepthIdx.empty())
        {
            ...
            for(size_t j=0; j<vDepthIdx.size();j++)
            {
                ...
                // 如果需要就新建地图点，这里的地图点 不是临时的，是全局地图中新建地图点，用于跟踪
                if(bCreateNew)
                {
                    cv::Mat x3D = mCurrentFrame.UnprojectStereo(i);
            ...
            std::cout << "             ::CreateNewKeyFrame()  插入关键帧时，生成地图点数量:" << new_mappoint
                      << "/" << nPoints << std::endl;
        }
    }
...
//            std::cout << pMP->mnId << std::endl;
    ...
    std::cout << "             ::TrackLocalMap()->SearchLocalPoints() 视野范围内的地图点数量: "
              << nToMatch << std::endl;
...
//        std::cout << mCurrentFrame.GetRotationInverse() << mvpLocalMapPoints.size();

        int nmatches=matcher.SearchByProjection(mCurrentFrame,mvpLocalMapPoints,th);
        std::cout << "             ::TrackLocalMap()->SearchLocalPoints() 对视野范围内的地图点通过投影进行特征点匹配数量: "
                  << nmatches << std::endl;
    }
}
...
void Tracking::UpdateLocalMap()
{
    ...
    UpdateLocalPoints();
    TicToc opt;
    UpdateLidarLocalPoints();
    std::cout << "             ::TrackLocalMap()->UpdateLocalMap() 局部关键帧数量: " << mvpLocalKeyFrames.size() << std::endl;
    std::cout << "             ::TrackLocalMap()->UpdateLocalMap() 局部地图点数量: " << mvpLocalMapPoints.size() << std::endl;
    std::cout << "             ::TrackLocalMap()->UpdateLocalMap() 局部lidar地图点数量: " << local_lidarmap_cloud_ptr_->size()
              << "            其耗时:" << opt.toc() << std::endl;
...
void Tracking::UpdateLocalKeyFrames()
{
    ...
    /// 改
    sort( mvpLocalKeyFrames.begin(), mvpLocalKeyFrames.end(), [](const KeyFrame* a,const KeyFrame* b)
                                                              {return a->mnId<b->mnId;} );

    // Include also some not-already-included keyframes that are neighbors to already-included keyframes
    // Step 2.2 策略2：遍历策略1得到的局部关键帧里共视程度很高的（前10个）关键帧，将他们的共视程度最高的邻居作为局部关键帧
    for(vector<KeyFrame*>::const_iterator itKF=mvpLocalKeyFrames.begin(), itEndKF=mvpLocalKeyFrames.end(); itKF!=itEndKF; itKF++)
    {
        ...
        for(vector<KeyFrame*>::const_iterator itNeighKF=vNeighs.begin(), itEndNeighKF=vNeighs.end(); itNeighKF!=itEndNeighKF; itNeighKF++)
        {
            ...
            if(!pNeighKF->isBad())
            {
                ...
                if(pNeighKF->mnTrackReferenceForFrame!=mCurrentFrame.mnId)
                {
                    ...
                    //? 找到一个就直接跳出for循环？将他们的共视程度最高的一个邻居作为局部关键帧
                    break;
                }
            }
        }
...
void Tracking::UpdateLidarLocalPoints(){
  //The key frame within 60 meters is selected as the local key frame of lidar
  PointIRTCloud local_less_corner_map;
  PointIRTCloud local_less_flat_map;
//  Eigen::Matrix<double,4,4> extrinsicMatrix=mpSystem->getExtrinsicMatrix();

//  std::vector<KeyFrame*> allKF=mpMap->GetAllKeyFrames();
//  cv::Mat curFpose=mCurrentFrame.mTcw.inv();
//  Eigen::Vector3d curFtrans(curFpose.at<float>(0,3), curFpose.at<float>(1,3), curFpose.at<float>(2,3));

  for(KeyFrame* KF:mvpLocalKeyFrames){
    cv::Mat KFpose=(KF)->GetPoseInverse();

    PointIRTCloud corner_points_less_sharp = (KF)->corner_points_less_sharp_;
    PointIRTCloud surface_points_less_flat = (KF)->surface_points_less_flat_;
    // calibration
//    pcl::transformPointCloud(corner_points_less_sharp, corner_points_less_sharp, extrinsicMatrix);
//    pcl::transformPointCloud(surface_points_less_flat, surface_points_less_flat, extrinsicMatrix);
    Eigen::Matrix<double,4,4> eigenKFpose;
    eigenKFpose << KFpose.at<float>(0,0), KFpose.at<float>(0,1), KFpose.at<float>(0,2),KFpose.at<float>(0,3),
    ...
    Eigen::Affine3d KFSE3(eigenKFpose);
    pcl::transformPointCloud(corner_points_less_sharp, corner_points_less_sharp, KFSE3);
    pcl::transformPointCloud(surface_points_less_flat, surface_points_less_flat, KFSE3);
    local_less_corner_map += corner_points_less_sharp;
    local_less_flat_map += surface_points_less_flat;
  }

  pcl::PointCloud<PointI>::Ptr lidarlocalmap_ptr=PointICloud().makeShared();

  for(PointIRT PIRT:local_less_flat_map){
    PointI p;
    p._PointXYZI::x=PIRT.x;
    p._PointXYZI::y=PIRT.y;
    p._PointXYZI::z=PIRT.z;
    p._PointXYZI::intensity=PIRT.intensity;
    local_lidarmap_cloud_ptr_->push_back(p);
  }

  pcl::VoxelGrid<pcl::PointXYZI> down_size_filter;
  down_size_filter.setInputCloud(local_lidarmap_cloud_ptr_);
  down_size_filter.setLeafSize(0.4, 0.4, 0.4);
  down_size_filter.filter(*local_lidarmap_cloud_ptr_);
  ...
  kdtree_local_map_->setInputCloud(local_lidarmap_cloud_ptr_);
    ...
    while(nCandidates>0 && !bMatch)
    {
        ...
        for(int i=0; i<nKFs; i++)
        {
            ...
            if(!Tcw.empty())
            {
                ...
                int nGood = Optimizer::PoseOptimization(&mCurrentFrame, local_lidarmap_cloud_ptr_, kdtree_local_map_, mpSystem->getLidarConfig());
                ...
                if(nGood<50)
                {
                    ...
                    if(nadditional+nGood>=50)
                    {
                        ...
                        nGood = Optimizer::PoseOptimization(&mCurrentFrame, local_lidarmap_cloud_ptr_, kdtree_local_map_, mpSystem->getLidarConfig());
...
void Tracking::UpdateViewer(){
    ...
    std::vector<MapPoint*> TrackedMapPoints = mCurrentFrame.mvpMapPoints;
    std::vector<cv::KeyPoint> TrackedKeyPoints = mCurrentFrame.mvKeys;   // is mvkeyun
    int N=TrackedMapPoints.size();
    KP_Viewer_.resize(N,false);
    for(int i=0; i<N; i++){
        if(TrackedMapPoints[i]&&!mCurrentFrame.mvbOutlier[i]&&TrackedMapPoints[i]->Observations()>0)
            KP_Viewer_[i]=true;
    }
}
