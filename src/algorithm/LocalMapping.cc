...
#include "algorithm/LocalMapping.h"
...
// 构造函数
LocalMapping::LocalMapping(Map *pMap, const float bMonocular,const lidarConfig** lidar_config):
    mbMonocular(1), mbResetRequested(false), mbFinishRequested(false), mbFinished(true), mpMap(pMap),
...
void LocalMapping::Run()
{
    ...
    while(1)
    {
        ...
        if(CheckNewKeyFrames())
        {
            ...
            ProcessNewKeyFrame();
            std::cout << "-----------------------------后端--------------------------" << std::endl;
            std::cout << "[LocalMapping](0)::等待处理的关键帧列表不为空!开始处理关键帧..." << std::endl;
            std::cout << "[LocalMapping](1)::计算关键帧BoW、更新观测、描述子、共视图，插入到地图！" << std::endl;
            ...
            std::cout << "[FusionSLAM]::局部地图优化耗时:" << timer.toc() << " ms." << std::endl;
...
void LocalMapping::MapPointCulling()
{
    ...
    std::cout << "[LocalMapping](2)::MapPointCulling() 删除地图点数量:" << badpoint_count << std::endl;
}
...
//        }
    ...
    std::cout << "[LocalMapping](3)::CreateNewMapPoints() 三角化新生成地图点数量:" << nnew << std::endl;
}
...
void LocalMapping::SearchInNeighbors()
{
    ...
    for(vector<KeyFrame*>::iterator vit=vpTargetKFs.begin(), vend=vpTargetKFs.end(); vit!=vend; vit++)
    {
        ...
        int repeatMPnum=matcher.Fuse(pKFi,vpMapPointMatches);
        repeatMPsum+=repeatMPnum;
    }
    ...
    int repeatMPnum = matcher.Fuse(mpCurrentKeyFrame,vpFuseCandidates);
    repeatMPsum+=repeatMPnum;
    std::cout << "                 ::SearchInNeighbors() 融合（重复）地图点数量:" << repeatMPnum << std::endl;
...
/**
 * @brief 检测当前关键帧在共视图中的关键帧，根据地图点在共视图中的冗余程度  剔除该共视关键帧
...
void LocalMapping::KeyFrameCulling()
{
    ...
    // pKF： mpCurrentKeyFrame的某一个共视关键帧，删除的也是共视关键帧
    ...
    for(vector<KeyFrame*>::iterator vit=vpLocalKeyFrames.begin(), vend=vpLocalKeyFrames.end(); vit!=vend; vit++)
    {
        ...
        {
            std::cout << "[LocalMapping](5)::删除冗余关键帧ID:" << pKF->mnId << std::endl;
            pKF->SetBadFlag();
        }
    }
}
