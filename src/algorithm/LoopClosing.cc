...
#include "algorithm/LoopClosing.h"
...
void LoopClosing::Run()
{
    ...
    while(is_loop_detection_)
    {
        ...
        {
            TicToc loop_timer;
            std::cout << "[LoopClosing]::关键帧不为空，开始回环检测！" << std::endl;
            // Detect loop candidates and check covisibility consistency
            if(DetectLoop())
            {
               std::cout << "[LoopClosing]::检测到候选关键帧，待进一步验证！" << std::endl;
               // Compute similarity transformation [sR|t]
               // In the stereo/RGBD case s=1
               if(ComputeSim3())
               {
                   std::cout << "[LoopClosing]::成功检测到回环！！！！" << std::endl;
                   // Perform loop fusion and pose graph optimization
                   CorrectLoop();

                   std::cout << "[FusionSLAM]::回环检测+全局BA优化耗时:" << loop_timer.toc() << " ms." << std::endl;
               }else{
                   std::cout << "[LoopClosing]::没有检测到回环" << std::endl;
               }
            }else{
                std::cout << "[LoopClosing]::没有检测到回环" << std::endl;
            }
...
//    std::cout << "[LoopClosing]::DetectLoop() 当前关键帧与相邻关键帧的最小bow相似度得分为:" << minScore << std::endl;
    ...
    vector<bool> vbConsistentGroup(mvConsistentGroups.size(),false);  //mvConsistentGroups 会在下面的if中第一次存入数据
    ...
    for(size_t i=0, iend=vpCandidateKFs.size(); i<iend; i++)
    {
        ...
        // 其中 ConsistentGroup的定义：typedef pair<set<KeyFrame*>,int> ConsistentGroup
        // 其中 ConsistentGroup.first对应每个“连续组”中的关键帧，ConsistentGroup.second为每个“连续组”的序号
...
bool LoopClosing::ComputeSim3()
{
    ...
    // Step 1. 遍历闭环候选帧集，初步筛选出与当前帧的匹配特征点数大于20的候选帧集合，并为每一个候选帧构造一个Sim3Solver
...
void LoopClosing::CorrectLoop()
{
    ...
    // Fuse duplications.
    // Step 4：将闭环相连关键帧组mvpLoopMapPoints 投影到当前关键帧组中，进行匹配，新增或替换当前关键帧组中KF的地图点
 ...
 * 而当前关键帧组中的关键帧的地图点是最近新计算的，可能有累积误差
 * 
 * @param[in] CorrectedPosesMap         待闭环矫正的当前KF对应的共视关键帧及Sim3变换
 */
void LoopClosing::SearchAndFuse(const KeyFrameAndPose &CorrectedPosesMap)
{
    ...
    for(KeyFrameAndPose::const_iterator mit=CorrectedPosesMap.begin(), mend=CorrectedPosesMap.end(); mit!=mend; mit++)
    {
        ...
        // vpReplacePoints：存储mvpLoopMapPoints投影到pKF匹配后需要替换掉的新增地图点,索引和mvpLoopMapPoints一致
        // 搜索区域系数为4
...
{
    cout << "[LoopClosing]::开始全局BA优化！！！！" << endl;
    ...
    // We need to propagate the correction through the spanning tree
    {
        ...
        // 如果没有中断当前次BA的请求
        // 这里和上面那句话的功能还有些不同,因为如果一次全局优化被中断,往往意味又要重新开启一个新的全局BA;
        // 为了中断当前正在执行的优化过程mbStopGBA将会被置位,同时会有一定的时间
        // 使得该线程进行响应;而在开启一个新的全局优化进程之前 mbStopGBA 将会被置为False
        // 因此,如果被强行中断的线程退出时已经有新的线程启动了,mbStopGBA=false,为了避免进行后面的程序,所以有了上面的程序;
        // 而如果被强行终端的线程退出时新的线程还没有启动,那么上面的条件就不起作用了
        // (虽然概率很小,前面的程序中mbStopGBA置位后很快mnFullBAIdx就++了,保险起见),所以这里要再判断一次
        if(!mbStopGBA)
        {
            cout << "[LoopClosing]::全局BA结束！！！！" << endl;
            cout << "[LoopClosing]::更新地图中..." << endl;
            mpLocalMapper->RequestStop();
