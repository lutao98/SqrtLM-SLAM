...
public:
  ...
  PoseLieCostFunction(const Eigen::Vector2d& observed, const Eigen::Vector3d& point,
                      const float information_matrix)
    : observed_(observed),point_(point),information_matrix_(information_matrix) {}
  ...
  bool operator()(const T* const pose_raw, T* residuals) const {
    ...
    residuals[0] = T(information_matrix_)*( P[0]*T(sfx)+T(scx) - T(observed_[0]) );
    residuals[1] = T(information_matrix_)*( P[1]*T(sfy)+T(scy) - T(observed_[1]) );
  ...
  static ceres::CostFunction *Create(const Eigen::Vector3d &pt3d, const Eigen::Vector2d &pt2d,
    ...
    return (new ceres::AutoDiffCostFunction<PoseLieCostFunction, 2, 7>(
              new PoseLieCostFunction(pt2d, pt3d, information_matrix)));
  }

private:
  const Eigen::Vector2d observed_;
  const Eigen::Vector3d point_;
  float information_matrix_;
};
