...
public:
     ...
     */
    ORBextractor(int nfeatures_, float scaleFactor_, int nlevels_, int iniThFAST_, int minThFAST_);
...
protected:
    ...
    int nfeatures_;			                    ///<整个图像金字塔中，要提取的特征点数目
    double scaleFactor_;		                    ///<图像金字塔层与层之间的缩放因子
    int nlevels_;			                    ///<图像金字塔的层数
    int iniThFAST_;			                    ///<初始的FAST响应值阈值
    int minThFAST_;			                    ///<最小的FAST响应值阈值

    std::vector<int> vFeaturesPerLevel_;		///<分配到每层图像中，要提取的特征点数目

    std::vector<int> umax_;	                    ///<计算特征点方向的时候，有个圆形的图像区域，
                                                ///这个vector中存储了每行u轴的边界（四分之一，其他部分通过对称获得）

    std::vector<float> vScaleFactor_;		    ///<每层图像的缩放因子
    std::vector<float> vInvScaleFactor_;        ///<以及每层缩放因子的倒数
    std::vector<float> vLevelSigma2_;		    ///<存储每层的sigma^2,即上面每层图像相对于底层图像缩放倍数的平方
    std::vector<float> vInvLevelSigma2_;	    ///<sigma平方的倒数
};
